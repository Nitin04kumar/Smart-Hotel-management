...
const ManagerReviewReplyPage = lazy(() => import('@/pages/manager/review-reply'));
const AdminDashboardPage = lazy(() => import('@/pages/admin/dashboard'));
const AdminApproveHotelsPage = lazy(() => import('@/pages/admin/approve-hotels'));
...
export default function AppDashboard() {
  ...
  useEffect(() => {
    if (section && !allowed.has(section)) navigate(`/app/${menu[0].key}`, { replace: true });
  }, [section, allowed, navigate, menu]);

  useEffect(() => {
    const label = menu.find((m) => m.key === current)?.label || (menu[0]?.label ?? '');
      ...
      <main className="app-main">
        ...
        >
          {role === 'user' ? (
            ...
            <section className="card" style={{ margin: 0 }}>
              <p className="muted">This section is under construction for {role}.</p>
            </section>
          )}
        </Suspense>
      </main>
